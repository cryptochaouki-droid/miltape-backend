/* =========================================================
SCRIPT CLIENT MILTAPE – Version finale (multi-crypto + notifications + tableau de bord + Telegram + ticker) CORRIGÉ
========================================================= */
...
const tapTimerDisplay = document.getElementById('tapTimerDisplay'); // ← AJOUT
...
function updateDemoUI() {
  if (demoMode) {
    ...
    if (!isSpectator && tapMessage) tapMessage.innerText = "🔬 MODE DÉMO ACTIF – Tu peux jouer sans payer !";
  } else {
    if (demoStatus) {
    ...
    if (!isSpectator && tapMessage) tapMessage.innerText = "⚡ CHOISIS TA MISE ET CONNECTE TON WALLET";
  }
}
updateDemoUI();
...
function joinSpectator() {
  ...
  if (tapMessage) tapMessage.innerText = "👁️ Mode spectateur : " + name + " regarde la partie !";
...
async function restoreSession() {
  ...
  if (spectator && name) {
    ...
    isPaid = true;
    if (tapButton) tapButton.disabled = true;
    if (tapMessage) tapMessage.innerText = "👁️ Spectateur : " + name;
    if (enterChallenge) enterChallenge.style.display = 'none';
    if (spectatorBtn) {
      spectatorBtn.classList.add('active');
      spectatorBtn.textContent = '👁️ SPECTATEUR ACTIF';
...
socket.on("player:joined", (data) => {
  if (data.success) {
    ...
    if (tapButton) tapButton.disabled = true;
    if (tapMessage) tapMessage.innerText = "💰 " + data.player.name + ", paie ta mise pour taper !";
    if (enterChallenge) enterChallenge.style.display = "none";
    if (tapCount) tapCount.innerText = data.player.taps;
    if (tapButtonCount) tapButtonCount.innerText = data.player.taps; // ⚠️ conservé pour l'instant
...
socket.on("spectator:joined", (data) => {
  if (data.success) {
    ...
    isPaid = true;
    if (tapButton) tapButton.disabled = true;
    if (tapMessage) tapMessage.innerText = "👁️ Vous regardez en direct (" + data.spectators + " spectateurs)";
    if (payButton) payButton.style.display = 'none';
    if (betInput) betInput.disabled = true;
...
socket.on("player:restored", (data) => {
  if (data.success) {
    ...
    if (demoMode) {
      ...
      isPaid = false;
      if (tapButton) tapButton.disabled = true;
      if (tapMessage) tapMessage.innerText = "💰 " + data.player.name + ", paie ta mise pour continuer à taper !";
      if (payButton) {
        payButton.style.display = 'block';
        payButton.onclick = () => initiatePayment(data.player.wallet, data.player.bet);
      }
...
socket.on("payment:verified", (data) => {
  if (data.verified && !isSpectator) {
    ...
    hapticSuccess();
    if (data.automatic) {
      if (tapMessage) tapMessage.innerText = "✅ Paiement automatique détecté ! Tape maintenant !";
    } else {
      if (tapMessage) tapMessage.innerText = "✅ Paiement vérifié ! Tape maintenant !";
    }
    if (payButton) payButton.style.display = 'none';
  }
});
...
socket.on("game:finished", (data) => {
  ...
  if (!isSpectator) {
    ...
    if (tapMessage) tapMessage.innerText = "🏆 Partie terminée ! " + (data.winners ? data.winners.length : 0) + " gagnant(s) !";
    alert(message);
    console.log(message);
  } else {
    if (tapMessage) tapMessage.innerText = "👁️ Partie terminée ! Prochaine partie dans 5 secondes...";
  }
...
function tapEffects(event) {
  ...
  tapButton.parentElement.appendChild(floatText);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    tapButton.parentElement.appendChild(particle);
  ...
  tapButton.parentElement.appendChild(ripple);
...
if (tapButton) {
  tapButton.addEventListener('click', function(event) {
    if (!isPlaying || tapButton.disabled || !isPaid || isSpectator) {
      if (!isPaid && !isSpectator && tapMessage) {
        tapMessage.innerText = "⏳ Tu dois payer ta mise avant de taper !";
        hapticError();
        setTimeout(() => {
          if (tapMessage) tapMessage.innerText = "💰 Paie ta mise pour taper !";
        }, 3000);
      }
      return;
    }
...
socket.on("player:score", (data) => {
  ...
  // ❌ SUPPRESSION de la mise à jour de tapButtonCount
...
socket.on("timer:update", (data) => {
  ...
  // ✅ Chrono en bas du bouton TAP
  if (tapTimerDisplay) {
    const minutes = Math.floor(data.remainingSeconds / 60);
    const seconds = data.remainingSeconds % 60;
    tapTimerDisplay.innerText = minutes + ":" + seconds.toString().padStart(2, '0');
  }
});
...
const modalOverlayEl = document.getElementById('dynamicModal');
if (modalOverlayEl) {
...
if (menuGamesBtn) {
  menuGamesBtn.addEventListener('click', function() {
    showMenuMessage('🎮 Mes parties',
      '<p>Tu n\'as pas encore de parties enregistrées.</p><p style="color:#888;font-size:12px;">Rejoins une partie pour commencer !</p>'
    );
  });
}
...
if (menuRankingsBtn) {
  menuRankingsBtn.addEventListener('click', function() {
    ...
    showMenuMessage('🏆 Mes classements',
      '<p>Ton meilleur score : <strong style="color:#ffcc00;">' + score + '</strong> taps</p><p style="color:#888;font-size:12px;">Continue à taper pour grimper dans le classement !</p>'
    );
  });
}
  ...
  menuGainsBtn.addEventListener('click', function() {
    ...
    if (!wallet && !playerId) {
      showMenuMessage('💰 Mes gains',
        '<p style="color:#888;">Tu n\'as pas encore de parties enregistrées.</p><p style="color:#888;font-size:12px;">Rejoins une partie pour commencer !</p>'
      );
      return;
    }
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        if (history.length === 0) {
          ...
          html += '<div class="history-table-wrapper" style="overflow-x:auto; -webkit-overflow-scrolling:touch;">' +
            '<table class="history-table" style="width:100%; border-collapse:collapse; font-size:11px; margin-top:5px;">' +
            '<thead><tr style="color:var(--gold); border-bottom:1px solid rgba(255,255,255,0.08);">' +
...
if (menuWithdrawalsBtn) {
  menuWithdrawalsBtn.addEventListener('click', function() {
    showMenuMessage('💸 Mes retraits',
      '<p>Tu n\'as pas encore effectué de retrait.</p><p style="color:#888;font-size:12px;">Les retraits sont disponibles après chaque partie.</p>'
    );
  });
}
...
if (menuReferralBtn) {
  menuReferralBtn.addEventListener('click', function() {
    showMenuMessage('👥 Parrainage',
      '<p>Parraine tes amis et gagne des bonus !</p><p style="color:#888;font-size:12px;">Lien de parrainage :<br><span style="color:#ffcc00;word-break:break-all;font-size:11px;">https://cryptochaouki-droid.github.io/miltape-backend/</span></p>'
    );
  });
}
...
socket.on("jackpot:update", (data) => {
  const prizeElement = document.getElementById('jackpotAmount'); // ✅ ID corrigé
