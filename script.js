/* =========================================================
MILTAPE WORLD CHALLENGE - BACKEND COMPLET CORRIGÉ
========================================================= */
...
async function finishGame() {
    ...
    io.emit("game:finished", { gameId: game.id, leaderboard, onlinePlayers: onlineSockets.size });
    await broadcastGameState();
...
app.get("/", (req, res) => {
    res.json({
        success: true,
        service: "Miltape World Challenge",
        status: "online",
...
app.get("/api/player/status", async (req, res) => {
    try {
        ...
        if (!playerId && !wallet) {
            return res.status(400).json({ success: false, message: "playerId ou wallet requis." });
        }
        ...
        if (!player) {
            return res.status(404).json({ success: false, message: "Aucun joueur trouvé pour cette partie." });
        }
...
app.post("/api/tap", async (req, res) => {
    try {
        if (game.status !== "running") {
            return res.status(400).json({ success: false, message: "La partie est terminée." });
        }
        ...
        if (!player || player.gameId !== game.id || getRemainingSeconds() <= 0) {
            return res.status(400).json({ success: false, message: "Joueur introuvable ou chrono terminé." });
        }
...
async function verifyUsdtTransaction(txId, expectedFrom, expectedAmount) {
    ...
    if (!Array.isArray(contracts) || contracts.length !== 1 || contracts[0].type !== "TriggerSmartContract") {
        throw new Error("Ce n'est pas une transaction USDT TRC20 valide.");
    }
    ...
    if (!data.startsWith("a9059cbb") || data.length < 136) throw new Error("Ce n'est pas un transfert USDT valide.");

    const recipientHex = "41" + data.substring(32, 72);
    const recipient = tronWeb.address.fromHex(recipientHex);
    if (!sameWallet(recipient, MILTAPE_WALLET)) throw new Error("Le paiement n'est pas destiné au wallet Miltape.");
    ...
    if (amount < requiredAmount) {
        throw new Error(`Montant insuffisant : ${amount} USDT reçu, ${requiredAmount} USDT requis.`);
    }

    const info = await tronWeb.trx.getTransactionInfo(cleanTxId);
    if (!info || !info.receipt || info.receipt.result !== "SUCCESS") {
        throw new Error("La transaction USDT n'est pas confirmée.");
    }
...
app.post("/api/payment/verify", async (req, res) => {
    try {
        ...
        if (!txId) {
            return res.status(400).json({ success: false, verified: false, message: "txId manquant." });
        }
        ...
        res.json({ success: true, verified: true, payment: { txId: result.txId, from: result.from, to: result.to, amount: result.amount } });
    } catch (error) {
...
app.get("/api/status", (req, res) => {
    res.json({
        ...
        wallet: MILTAPE_WALLET,
        game: { id: game.id, status: game.status, remainingSeconds: getRemainingSeconds() },
        onlinePlayers: onlineSockets.size
    });
});
...
server.listen(PORT, async () => {
    console.log("");
    console.log("==============================================");
    console.log("       🚀 MILTAPE BACKEND ONLINE");
    console.log("==============================================");
    console.log("🌐 Port :", PORT);
    console.log("💰 Wallet :", MILTAPE_WALLET);
    console.log("💵 USDT :", USDT_CONTRACT);
    console.log("🎮 Jeu : 10 minutes");
    console.log("🏆 Top 5 actif");
