...
// ============================================================
// ✅ ROUTE HEALTHCHECK AVANT LE RATE LIMITER (Pour éviter les erreurs 429)
// ============================================================
...
// ============================================================
// ✅ RATE LIMITER (On ignore /api/status)
// ============================================================
...
// CONNEXION MONGO
// ============================================================
mongoose.connect(MONGODB_URI).then(() => console.log("✅ MongoDB connecté.")).catch((error) => {
...
async function drawJackpot() {
    ...
    if (jackpot.participants.length === 0) {
        console.log("🎁 Pas de participants pour la cagnotte.");
        jackpot.drawn = true; jackpot.prize = 0; jackpot.accumulatedFund = 0;
        await jackpot.save(); await emitJackpotUpdate();
        return;
    }
    ...
    if (winner && prizeAmount > 0) await sendTokenToWinner(winner.wallet, prizeAmount, 'USDT');
    await emitJackpotUpdate();
}
        ...
        for (const tx of transactions) {
            ...
            if (contract.type === "TransferContract") {
                ...
                for (const [sym, info] of Object.entries(SUPPORTED_TOKENS)) {
                    if (info.contract && sameWallet(contractAddress, info.contract)) { foundToken = sym; decimals = info.decimals; break; }
                }
            ...
            const matchingPlayer = unpaidPlayers.find(p => sameWallet(p.wallet, ownerAddress) && p.bet > 0 && p.token === token && amount >= p.bet);
            if (!matchingPlayer) continue;
            matchingPlayer.paid = true;
            matchingPlayer.paymentTxId = txId;
            await matchingPlayer.save();
            processedTxIds.add(txId);
            await Payment.create({ txId, from: ownerAddress, to: MILTAPE_WALLET, amount, verified: true, gameId: game.id, token });
...
async function finishGame() {
    ...
    for (const w of winners) {
        ...
        await History.create({ playerId: w._id, playerName: w.name, wallet: w.wallet, gameId: game.id, rank: w.rank, bet: w.bet, gain: w.gain, taps: w.taps, token: w.token });
    }
    ...
    if (realPayments.length > 0) {
        console.log("💸 Envoi des gains...");
        for (const w of winners) { if (w.gain > 0) await sendTokenToWinner(w.wallet, w.gain, w.token); }
    } else {
        console.log("🔬 Mode démo : transferts simulés.");
    }
    await broadcastGameState();
    await emitJackpotUpdate();
...
io.on("connection", async (socket) => {
    ...
    socket.on("player:join", async (data) => {
        try {
            ...
            if (!player) {
                ...
                player.name = name; player.bet = bet; player.token = token;
                await player.save();
            }
            ...
            socket.emit("player:joined", { success: true, player: { id: player._id, name: player.name, wallet: player.wallet, taps: player.taps, bet: player.bet, paid: player.paid, token: player.token } });
    ...
    socket.on("spectator:join", async (data) => {
        try {
            ...
            await broadcastGameState();
        } catch (error) { console.error("spectator:join:", error.message); socket.emit("error", { message: "Impossible de rejoindre en spectateur." }); }
    });
    ...
    socket.on("player:restore", async (data) => {
        try {
            ...
            if (playerId) query._id = playerId; else query.wallet = wallet;
            ...
            socket.data.isSpectator = false;
            socket.emit("player:restored", { success: true, player: { id: player._id, name: player.name, wallet: player.wallet, taps: player.taps, bet: player.bet, paid: player.paid, token: player.token || 'USDT' } });
            const leaderboard = await getLeaderboard();
        ...
        } catch (error) { console.error("player:restore:", error.message); socket.emit("error", { message: "Erreur restauration." }); }
    });
...
// ============================================================
// OPTIMISATION : CLASSEMENT GLOBAL TOUTES LES 2 SECONDES
// ============================================================
setInterval(async () => {
    try {
    ...
    } catch (error) { console.error("Erreur classement global :", error.message); }
}, 2000);
...
app.get("/api/admin/stats", async (req, res) => {
    try {
        ...
        res.json({ success: true, recentPlayers: recentPlayers.map(p => ({ playerName: p.name, playerId: p._id, score: p.taps, wallet: p.wallet, token: p.token || 'USDT' })) });
    } catch (error) { console.error("/api/admin/stats:", error.message); res.status(500).json({ success: false, message: error.message }); }
});

app.get("/api/admin/payouts", async (req, res) => {
    try {
        const winners = await Player.find({ gameId: game.id }).sort({ taps: -1 }).limit(5).select('name wallet taps bet token');
        res.json({ success: true, winners: winners.map((p, i) => ({ rank: i + 1, playerName: p.name, wallet: p.wallet, score: p.taps, amount: p.bet || 0, token: p.token || 'USDT' })) });
    } catch (error) { console.error("/api/admin/payouts:", error.message); res.status(500).json({ success: false, message: error.message }); }
});

app.get("/api/game", async (req, res) => {
    try {
        const leaderboard = await getLeaderboard();
        res.json({ success: true, gameId: game.id, status: game.status, startedAt: game.startedAt, endsAt: game.endsAt, durationSeconds: game.durationSeconds, remainingSeconds: getRemainingSeconds(), onlinePlayers: onlineSockets.size, spectators: spectatorSockets.size, leaderboard });
    } catch (error) { console.error("/api/game:", error.message); res.status(500).json({ success: false, message: "Erreur serveur." }); }
});
...
app.get("/api/total-stakes", async (req, res) => {
    ...
    } catch (error) { console.error("/api/total-stakes:", error.message); res.status(500).json({ success: false, message: error.message }); }
});
...
app.post("/api/join", async (req, res) => {
    try {
        ...
        let player = await Player.findOne({ gameId: game.id, wallet });
        if (!player) { player = await Player.create({ gameId: game.id, name, wallet, taps: 0, bet, paid: false, token }); }
        else { player.name = name; player.bet = bet; player.token = token; await player.save(); }
        const jackpot = await getCurrentJackpot();
        if (jackpot && !jackpot.drawn && !jackpot.participants.includes(player._id)) { jackpot.participants.push(player._id); await jackpot.save(); }
        res.json({ success: true, player: { id: player._id, name: player.name, wallet: player.wallet, taps: player.taps, bet: player.bet, paid: player.paid, token: player.token } });
    } catch (error) { console.error("/api/join:", error.message); res.status(500).json({ success: false, message: "Erreur serveur." }); }
});
...
app.get("/api/player/status", async (req, res) => {
    try {
        ...
        if (playerId) query._id = playerId; else query.wallet = wallet;
        const player = await Player.findOne(query);
        if (!player) return res.status(404).json({ success: false, message: "Aucun joueur trouvé." });
        res.json({ success: true, player: { id: player._id, name: player.name, wallet: player.wallet, taps: player.taps, bet: player.bet, paid: player.paid, token: player.token || 'USDT' } });
    } catch (error) { console.error("/api/player/status:", error.message); res.status(500).json({ success: false, message: "Erreur serveur." }); }
});
...
app.post("/api/tap", async (req, res) => {
    try {
        ...
        player.taps += 1; await player.save();
        const leaderboard = await getLeaderboard();
        io.emit("leaderboard:update", leaderboard);
        res.json({ success: true, taps: player.taps, leaderboard });
    } catch (error) { console.error("/api/tap:", error.message); res.status(500).json({ success: false, message: "Erreur serveur." }); }
});
...
app.get("/api/chat", async (req, res) => {
    try {
        const messages = await Message.find({ $or: [{ gameId: game.id }, { gameId: null }] }).sort({ createdAt: -1 }).limit(50).lean();
        messages.reverse();
        res.json({ success: true, messages });
    } catch (error) { console.error("/api/chat:", error.message); res.status(500).json({ success: false, message: "Erreur serveur." }); }
});
...
app.get("/api/jackpot", async (req, res) => {
    try {
        ...
        let winner = null;
        if (jackpot.winner) { const w = await Player.findById(jackpot.winner).select('name'); if (w) winner = w.name; }
        res.json({ success: true, jackpot: { prize: jackpot.prize, participants: jackpot.participants.length, nextDraw: nextDraw, drawn: jackpot.drawn, winner } });
    } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});

app.get("/api/jackpot/history", async (req, res) => {
    try {
        const history = await Jackpot.find({ drawn: true }).sort({ weekEnd: -1 }).limit(20).populate('winner', 'name');
        res.json({ success: true, history: history.map(j => ({ week: j.weekStart.toDateString() + ' - ' + j.weekEnd.toDateString(), prize: j.prize, winner: j.winner ? j.winner.name : null })) });
    } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});
...
app.post("/api/payment/verify", async (req, res) => {
    try {
        ...
        if (!player) return res.status(404).json({ success: false, message: "Joueur introuvable." });
        io.emit("payment:verified", { verified: true, wallet: player.wallet, amount: amount || player.bet, playerName: player.name, automatic: true, token: player.token || 'USDT' });
        res.json({ success: true, verified: true, message: "Paiement démo validé." });
    } catch (error) { console.error("Erreur /api/payment/verify:", error.message); res.status(500).json({ success: false, message: error.message }); }
});
...
app.get("/api/player/history", async (req, res) => {
    try {
        ...
        res.json({ success: true, player: { wallet: wallet || "inconnu", totalGain, gamesPlayed, bestScore }, history: history.map(h => ({ gameId: h.gameId, rank: h.rank, bet: h.bet, gain: h.gain, taps: h.taps, token: h.token || 'USDT', createdAt: h.createdAt })) });
    } catch (error) { console.error("Erreur /api/player/history:", error.message); res.status(500).json({ success: false, message: error.message }); }
});
