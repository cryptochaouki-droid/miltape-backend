...
async function tryStartDuel(matchId) {
    ...
    io.to(entry1.socketId).emit("duel:started", { 
        opponentName: player2.name, 
        bet, 
        prize: winnerPrize 
    });
    io.to(entry2.socketId).emit("duel:started", { 
    ...
    setTimeout(async () => {
        ...
        if (winner) {
            ...
            if (loser) await DuelEntry.create({ 
                playerId: loser._id, 
                bet: duel.bet, 
                token: "USDT", 
                paid: false 
...
io.on("connection", async (socket) => {
    ...
    socket.on("duel:join", async (data) => {
        ...
        if (duelPools[bet].length >= 2) {
            ...
            pendingDuelPayments[entry1.socketId] = { 
                bet, 
                playerId: entry1.playerId, 
                matchId, 
                duelPaid: false 
            };
            pendingDuelPayments[entry2.socketId] = { 
                bet, 
            ...
            io.to(entry1.socketId).emit("duel:need_payment", { 
                amount: bet, 
                wallet: MILTAPE_WALLET 
            });
            io.to(entry2.socketId).emit("duel:need_payment", { 
                amount: bet, 
                wallet: MILTAPE_WALLET 
            });
            ...
            const paymentTimeout = setTimeout(() => {
                ...
                io.to(entry1.socketId).emit("duel:cancelled", { 
                    message: state1?.duelPaid ? "Ton adversaire n'a pas payé à temps. Tu es remis en file d'attente." : "Tu n'as pas payé à temps. Le duel est annulé." 
                });
                io.to(entry2.socketId).emit("duel:cancelled", { 
                    message: state2?.duelPaid ? "Ton adversaire n'a pas payé à temps. Tu es remis en file d'attente." : "Tu n'as pas payé à temps. Le duel est annulé." 
                });
    ...
    socket.on("duel:tap", () => {
        for (const duelId in activeDuels) {
            ...
            if (socket.id === duel.socket1) {
                duel.taps1++;
                io.to(duel.socket1).emit("duel:score", { 
                    myTaps: duel.taps1, 
                    opponentTaps: duel.taps2 
                });
                io.to(duel.socket2).emit("duel:score", { 
                    myTaps: duel.taps2, 
                    opponentTaps: duel.taps1 
                });
                break;
...
app.post("/api/demo/verify", async (req, res) => {
    try {
        ...
        if (player.gameId !== game.id) return res.status(409).json({ 
            success: false, 
...
app.get("/api/wallet", (req, res) => res.json({ 
    success: true, 
    wallet: MILTAPE_WALLET 
}));
